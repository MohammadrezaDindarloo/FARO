...
| 19 | §II-C 2 (Eq. 9) | **Which collision pairs.** The paper says only *"all bodies in the robot and scene are modeled using convex geometries"* and *"for each collision pair A and B"* — it never defines the set, never mentions self-collision, and never says what happens to a pair the contact mode requires to be TOUCHING. That last omission is the substantive one. | **To build at Milestone 3.** Measured on the G1: `addAllCollisionPairs` gives **615** pairs; 12 collide at the nominal pose and **all 12 are adjacent** (parent–child links that touch by construction), so excluding adjacency leaves a genuinely self-collision-free nominal pose and 603 candidate pairs. No SRDF ships with the G1, so the disable list must be derived, not loaded. Environment (floor, tabletop) and objects (box) are **not** in the robot's `GeometryModel` and need pairs added explicitly. **The mode-dependent exclusion is mandatory, not an optimisation:** a sole resting on the floor measures `sd = 0.000000` exactly, and Eq. 7a *requires* that while Eq. 9 requires `sd ≥ margin` — compatible only at exactly zero, and with any positive margin never. So Eq. 9's pair set must be `all_pairs − adjacent − active_contacts(mode)`. | `configs/scenes/` → collision pair policy (Milestone 3) |
